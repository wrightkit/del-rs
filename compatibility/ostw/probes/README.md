...
Small pinned-reference probes for DEL/OSTW behaviors. These source probes were
authored in Wright and migrated here with their recorded observations; the
immutable reference identity and probe provenance remain unchanged. Every
probe runs the pinned OSTW v3.4.0 language server
...
The `deltin-rs` signature table derives its canonical argument order, defaults,
and en-US spellings from these recorded
...
`p6-catalog-signatures` (68 elements). They form the immutable, deltin-rs-owned
forward-compilation comparison boundary for the DEL/OSTW integration. A
downstream consumer can compile each one natively and validate semantic

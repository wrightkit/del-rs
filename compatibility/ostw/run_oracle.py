...
def progress(message): print(f"ostw-oracle: {message}", file=sys.stderr, flush=True)

def probe_reference(reference):
    return {
        "name": reference["name"],
        "version": reference["version"],
        "contentCommit": reference["contentCommit"],
        "assetSha256": reference["asset"]["sha256"],
    }

def require(condition, message):
    if not condition: raise OracleError(f"OSTW_EVIDENCE_INVALID: {message}")

def within(path, root):
    try: path.relative_to(root)
    except ValueError as error: raise OracleError(f"OSTW_EVIDENCE_INVALID: path escapes root: {path}") from error

def check_reference(reference):
    require(reference.get("schemaVersion") == 1, "unsupported reference schema")
    require(reference.get("name") == "ostw", "reference name is not ostw")
    require(reference.get("version") == "v3.4.0", "reference version is not v3.4.0")
    require(reference.get("repository") == "https://github.com/ItsDeltin/Overwatch-Script-To-Workshop", "reference repository changed")
    require(re.fullmatch(r"[0-9a-f]{40}", reference.get("contentCommit", "")) is not None, "reference content commit is not immutable")
    require(reference.get("ping") == "Hello!", "reference ping contract changed")
    asset = reference.get("asset", {})
    require(asset.get("name") == "v3.4.0-linux-x64.zip", "reference asset name changed")
    require(asset.get("target") == "linux-x64", "reference asset target changed")
    require(asset.get("executable") == "Deltinteger", "reference executable changed")
    require(isinstance(asset.get("size"), int) and asset["size"] > 0, "reference asset size is invalid")
    require(re.fullmatch(r"[0-9a-f]{64}", asset.get("sha256", "")) is not None, "reference asset SHA-256 is not immutable")
    require(asset.get("url", "").endswith("/v3.4.0/v3.4.0-linux-x64.zip"), "reference asset URL is not pinned")

def check_recorded_evidence(reference):
    check_reference(reference)
    corpus = load(CORPUS)
    require(corpus.get("schemaVersion") == 2, "unsupported corpus schema")
    projects = {}
    for project in corpus.get("projects", []):
        project_id = project.get("id")
        require(isinstance(project_id, str) and project_id, "corpus project has no id")
        require(project_id not in projects, f"duplicate corpus project {project_id}")
        project_root = (ROOT / project["path"]).resolve()
        within(project_root, ROOT)
        require(project_root.is_dir(), f"missing corpus project {project_id}")
        files = {}
        for item in project.get("files", []):
            path = (project_root / item["path"]).resolve()
            within(path, project_root)
            require(path.is_file(), f"missing corpus file {project_id}/{item['path']}")
            require(sha(path) == item["sha256"], f"corpus hash changed: {project_id}/{item['path']}")
            files[item["path"]] = path
        require(project["entry"] in files, f"corpus entry is not listed: {project_id}/{project['entry']}")
        roots = {root["root"]: root for root in project.get("roots", [])}
        require(len(roots) == len(project.get("roots", [])), f"duplicate corpus root: {project_id}")
        for root in roots:
            require(root in files, f"corpus root is not listed: {project_id}/{root}")
        projects[project_id] = {"manifest": project, "files": files, "roots": roots}
    require(projects, "corpus has no projects")

    results = load(RESULTS)
    require(results.get("schemaVersion") == 2, "unsupported corpus result schema")
    require(results.get("reference") == reference, "corpus results use a different reference")
    result_projects = {project["id"]: project for project in results.get("projects", [])}
    require(len(result_projects) == len(results.get("projects", [])), "duplicate corpus result project")
    require(set(result_projects) == set(projects), "corpus result project set differs from manifest")
    for project_id, project_info in projects.items():
        recorded = result_projects[project_id]
        require(recorded["entry"] == project_info["manifest"]["entry"], f"corpus result entry differs: {project_id}")
        observations = {observation["root"]: observation for observation in recorded.get("observations", [])}
        require(set(observations) == set(project_info["roots"]), f"corpus result roots differ: {project_id}")
        for root, observation in observations.items():
            require(observation.get("openMode") == "single", f"corpus observation is not single-root: {project_id}/{root}")
            require(observation.get("opened") == [root], f"corpus observation opened unexpected files: {project_id}/{root}")
            require(observation.get("accept") in ("accept", "reject"), f"invalid corpus result: {project_id}/{root}")
            for closure_path in observation.get("importClosure", []):
                require(closure_path in project_info["files"], f"result closure file is not in manifest: {project_id}/{closure_path}")

    manifests = {}
    target_ids = []
    for probe_dir in sorted(PROBES.iterdir()):
        manifest_path = probe_dir / "probe.json"
        if not probe_dir.is_dir() or not manifest_path.is_file(): continue
        manifest = load(manifest_path)
        probe_id = manifest.get("id")
        require(probe_id and probe_id not in manifests, f"invalid or duplicate probe id: {probe_id}")
        require(manifest.get("schemaVersion") == 1, f"unsupported probe schema: {probe_id}")
        require(manifest.get("reference") == probe_reference(reference), f"probe uses a different reference: {probe_id}")
        entry = (probe_dir / manifest["entry"]).resolve()
        within(entry, probe_dir.resolve())
        require(entry.is_file(), f"missing probe entry: {probe_id}")
        run_ids = set()
        for run in manifest.get("runs", []):
            run_id = run.get("runId")
            require(run_id and run_id not in run_ids, f"invalid or duplicate probe run: {probe_id}/{run_id}")
            run_ids.add(run_id)
            recorded_path = probe_dir / f"result.{run_id}.json"
            require(recorded_path.is_file(), f"missing probe result: {probe_id}/{run_id}")
            recorded = load(recorded_path)
            require(recorded.get("runId") == run_id and recorded.get("openMode") == run["openMode"], f"probe result metadata differs: {probe_id}/{run_id}")
            workshop_path = probe_dir / f"workshop.{run_id}.txt"
            if recorded.get("workshopAvailable"):
                require(workshop_path.is_file(), f"missing accepted probe workshop output: {probe_id}/{run_id}")
            if recorded.get("workshopCodeSha256") and workshop_path.is_file():
                require(sha(workshop_path) == recorded["workshopCodeSha256"], f"probe workshop hash changed: {probe_id}/{run_id}")
        manifests[probe_id] = manifest
        if manifest.get("role") == "differential-target": target_ids.append(probe_id)
    probe_results = load(PROBE_RESULTS)
    require(probe_results.get("schemaVersion") == 2, "unsupported probe result schema")
    require(probe_results.get("reference") == reference, "probe results use a different reference")
    require(probe_results.get("differentialTargets") == sorted(target_ids), "differential target set differs from manifests")
    result_probes = {probe["id"]: probe for probe in probe_results.get("probes", [])}
    ...
    require(boundary.get("surface") == "deltin-rs-ostw-reconstruct-v1", "reconstruction boundary is not owner-scoped")
    progress(f"validated {len(projects)} corpus projects, {len(manifests)} probes, and reconstruction boundary")
...
def acquire(reference, root):
    ...
    if not executable.is_file():
        progress(f"extracting {asset['name']}")
...
def ping(executable, reference):
    progress("pinging pinned reference")
    try:
        result = subprocess.run(command(executable)+["--ping"], capture_output=True, text=True, encoding="utf-8", timeout=30)
    except subprocess.TimeoutExpired as error:
        raise OracleError("OSTW_REFERENCE_TIMEOUT: --ping exceeded 30 seconds") from error
        ...
        if proc.poll() is None:
            ...
            try:
                ...
                try:
                    ...
                    raise OracleError("OSTW_ORACLE_SESSION_CLOSED: langserver did not terminate") from kill_error
    return messages
...
def run_probe(executable, reference, probe_dir):
    ...
    for run in manifest["runs"]:
        ...
        progress(f"probe {manifest['id']} / {run_id}")
        if mode == "all":
...
def main():
    parser=argparse.ArgumentParser(description="Reproduce or validate pinned OSTW evidence.")
    parser.add_argument("--reference-root", type=Path, default=ROOT/"target/ostw-reference")
    parser.add_argument("--acquire",action="store_true", help="download and verify the pinned Linux reference")
    parser.add_argument("--ping",action="store_true", help="only run the reference ping")
    parser.add_argument("--update",action="store_true", help="replace recorded observations after review")
    parser.add_argument("--probes",action="store_true", help="run pinned probe observations")
    parser.add_argument("--check",action="store_true", help="validate committed evidence without the upstream binary")
    args=parser.parse_args()
    try:
        reference=load(REFERENCE)
        if args.check:
            check_recorded_evidence(reference)
            return 0
        executable = acquire(reference,args.reference_root) if args.acquire else args.reference_root/reference["asset"]["executable"]
    ...
    except (OracleError, OSError, ValueError, KeyError, subprocess.SubprocessError) as error:
